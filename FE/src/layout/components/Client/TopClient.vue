  ...
  <header class="header-nav py-3 border-bottom bg-white shadow-sm sticky-top">
    <div class="container d-flex align-items-center justify-content-between">
      <div class="logo d-flex align-items-center">
        <img
          src="../../../assets/images/logo.pnj.png"
          alt=""
          class="img-fluid"
          style="height: 60px; width: 60px"
        />
        <router-link to="/" class="h3 fw-bold mb-0 text-orange">
          AI-Meet</router-link
        >
      </div>
      ...
      <div class="auth-buttons d-flex gap-2 align-items-center">
        <template v-if="!isLoggedIn">
          ...
          <router-link
            ...
            class="btn-orange rounded-pill px-4"
            >Đăng ký</router-link
          >
        </template>
        <template v-else>
          <router-link
            v-if="isDoiTac == 0"
            to="/nguoi-dung/trang-chinh"
            class="btn-text"
            >Trang của tôi</router-link
          >
          <router-link
            v-else-if="isDoiTac == 1"
            to="/doi-tac/trang-chinh"
            class="btn-text"
            >Trang đối tác</router-link
          >
          <a
            href="#"
...
export default {
  ...
  methods: {
    checkLoginStatus() {
      ...
      const tokenNguoiDung = localStorage.getItem("token_nguoi_dung");
